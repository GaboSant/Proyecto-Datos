...
// A partir de un string que viene en base binaria retorna lo que seria pero en
// base 32
...
class Nodo_a {
    ...
    void add_coord(Coordenada c){
        coord->add(c);
    }

    void add_hijo(Nodo_a* h){
        if (hijos->get_size() == 0) {
            hijos->add(h);
        } else {
            int i = 1;
    ...
    Nodo_a* get_hijo(string h) {
        if (hijos->get_size() == 0) {
            return NULL;
        } else {
            Nodo<Nodo_a*>* hijo = hijos->get_nodo(0);
            while (hijo != NULL) {
                string s = hijo->get_dato()->get_hash();
                if (s[s.length()-1] == h[0]) {
    ...
    Lista<Coordenada> *get_coord() { return coord; }
    Lista<Nodo_a *> *get_hijos() { return hijos; }
};
...
class Arbol {
    ...
    void add(Coordenada c) {
        string g = GeoHash(c.get_x(), c.get_y(), digitos);
        add(c, raiz, g, 0);
    }

    void add(Coordenada c, Nodo_a* nodo, string g, int i) {
        string s;
        s = g[i];
...
int main() {
    ...
    std::cout << "El tiempo de ejecución fue: " << tiempoEjecucion << " segundos." << std::endl;
